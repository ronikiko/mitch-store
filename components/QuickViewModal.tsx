...
const QuickViewModal: React.FC<QuickViewModalProps> = ({ product, isOpen, onClose, onAddToCart }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      {/* Modal Content */}
      <div className="relative bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col md:flex-row max-h-[90vh] md:h-auto">
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col overflow-y-auto custom-scrollbar">
            ...
            <div className="flex gap-2">
              {PACK_OPTIONS.map((count) => (
                <button
                  ...
                  className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-lg border text-sm font-medium transition-all ${
                    selectedPack === count 
          ...
          <button
            ...
            disabled={isAdding}
            className={`w-full py-4 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-all shadow-lg transform active:scale-[0.98] ${
              isAdding 
